...
## 2026-08-04 — Story 8.2 scoping (shared clinic resources)
Story's own spec is thin — one Gherkin scenario, one line of technical notes ("extends the availability/slot model in `booking` with a resource dimension"), no architecture/system-design/UX/UI doc coverage at all. BRAINSTORM offered three scope options (🟢 admin manually assigns a room after booking; 🟡 auto-assigned at booking time via the doctor's availability rule; 🔴 full resource model with rooms+equipment as separate types, multi-resource requirements, admin utilization view). User picked 🔴 the full model, overriding the 🟢 recommendation — this roughly doubles the story's nominal Size-M effort (8-batch plan, comparable to Epic 6+7).
