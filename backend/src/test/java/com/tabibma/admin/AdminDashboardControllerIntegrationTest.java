...
/** Story 10.3. Asserts deltas rather than absolute counts — {@code AbstractIntegrationTest}'s
 * singleton Testcontainers database is shared across every test class in a single test run
 * (see the 2026-07-29 fix in .logs/decisions.md), so other classes' bookings are already present
 * by the time this test runs. Same fix pattern as the e2e suite's dashboard/queue assertions. */
class AdminDashboardControllerIntegrationTest extends AbstractIntegrationTest {
    ...
    @Test
    void getHealth_rejectsNonPlatformAdmin() throws Exception {
        registerAndLogin("health-doctor1@example.com", "DOCTOR");
        String doctorToken = login("health-doctor1@example.com");
    ...
    @Test
    void getHealth_reflectsARealConfirmedBookingAndItsSucceededPayment() throws Exception {
        String adminToken = createPlatformAdminAndLogin("health-admin1@example.com");
        JsonNode before = getHealth(adminToken);

        String doctorToken = registerDoctorWithSlot("health-doctor2@example.com", DayOfWeek.THURSDAY);
        registerAndLogin("health-patient1@example.com", "PATIENT");
        String patientToken = login("health-patient1@example.com");
        bookOnlyOpenSlot(doctorToken, patientToken);

        JsonNode after = getHealth(adminToken);

        assertThat(after.get("totalAppointments").asLong() - before.get("totalAppointments").asLong()).isEqualTo(1);
        assertThat(after.get("confirmedAppointments").asLong() - before.get("confirmedAppointments").asLong())
                .isEqualTo(1);
        assertThat(after.get("succeededPayments").asLong() - before.get("succeededPayments").asLong()).isEqualTo(1);
    }

    private JsonNode getHealth(String adminToken) throws Exception {
        var result = mockMvc.perform(get("/api/v1/admin/platform/health")
    ...
    private void approveProfile(String profileId) throws Exception {
        String adminEmail = "platform-admin-health-" + profileId + "@example.com";
