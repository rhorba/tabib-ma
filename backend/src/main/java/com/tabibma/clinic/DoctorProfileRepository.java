...
public interface DoctorProfileRepository extends JpaRepository<DoctorProfile, UUID> {
    ...
    @Query("SELECT d FROM DoctorProfile d WHERE d.verificationStatus = com.tabibma.clinic.VerificationStatus.APPROVED "
            ...
            + "AND (:maxFeeMad IS NULL OR d.consultationFeeMad <= :maxFeeMad)")
    Page<DoctorProfile> search(@Param("specialty") String specialty, @Param("city") String city,
                                @Param("maxFeeMad") BigDecimal maxFeeMad, Pageable pageable);
}
